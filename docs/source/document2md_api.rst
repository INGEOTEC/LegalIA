...
Version |document2md_version| — see :doc:`index` for the full package table.
...
job; :py:mod:`nota2md` is what calls into :py:mod:`document2md` at all, and only
...
and :py:class:`~document2md.BatchConverter` (:py:mod:`document2md.batch`) used directly
...
example" in this whole epic.** Entering :py:class:`~document2md.BatchConverter`
...
``.readthedocs.yaml``/``test.yml`` install ``document2md`` with ``--no-deps``; see
...
``packages/document2md/tests/test_batch.py`` and ``test_cli.py``, which mock only
...
:py:class:`~document2md.BatchConverter` is the package's public entry point when
...
:py:class:`~document2md.BatchConverter` as its own ``converter`` parameter, so a
