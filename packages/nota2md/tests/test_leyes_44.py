...
* ligie_2022 (the import/export tariff schedule) is not organized into
  articles past its first one — the "law" is a many-thousand-row tariff
  table — so the article-by-article model barely applies to it.
* lspcapf's 2003 original publication numbers its articles as bare "1.",
  "2." rather than "Artículo 1.", which _ARTICULO does not match, so almost
  nothing in it is recognized as an article to begin with.
...
UMBRAL_EXCEPCIONES gives those specific laws a floor derived from where they
actually land (with a little room below it), so a real further regression in
one of them still fails the test without the general floor having to be
lowered enough to hide it for every other law too. UMBRAL_POR_LEY is that
general floor; UMBRAL_PROMEDIO catches a regression too small for any one
law's own floor to trip.
"""
