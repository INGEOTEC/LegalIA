...
set turned up one way it falls short of the real text that is not worth
...
lcid, lfcpo, lgsna and lgmsv used to need their own exceptions here too, for
a shared reason: a reform that restates only the fracciones/incisos it
changes and elides the rest with "..." was read as the article's whole new
text, so whatever the ellipsis stood for got silently dropped.
`_fusiona_articulo()` (see nota2md.leyes) now fills those placeholders back
in from the article's own previous text instead, which is why all four clear
the general floor on their own and no longer need a lowered one.

UMBRAL_EXCEPCIONES gives lspcapf's own shortfall a floor derived from where
it actually lands (with a little room below it), so a real further
regression there still fails the test without the general floor having to be
lowered enough to hide it for every other law too.
...
from nota2md.leyes import normative_reconstruction, normaliza_para_comparar
...
UMBRAL_POR_LEY = 0.55
# Known, explained shortfall (see the module docstring) that would otherwise
