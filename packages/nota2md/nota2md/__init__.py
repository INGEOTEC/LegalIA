...
(`download_legal_provisions_provenance_ids`), fetch a day's browsable
legal-provisions index (`fetch_daily_legal_provisions`), and download every
legal provision ever published, as titles (`download_legal_provisions_titles`)
— the package's five entry points, re-exported here so each can be imported
straight off `nota2md` (``from nota2md import legal_provisions``) instead of
its own submodule.
"""
