...
from document2md.mineru_server import ENV_VAR, MineruServer, _wait_until_healthy
...
class TestWaitUntilHealthy(unittest.TestCase):
    @patch("document2md.mineru_server.requests.get", return_value=_fake_response(200))
    ...
    @patch("document2md.mineru_server.time.sleep")
    @patch("document2md.mineru_server.requests.get", return_value=_fake_response(503))
...
class TestMineruServer(unittest.TestCase):
    ...
    @patch("document2md.mineru_server._wait_until_healthy")
    @patch("document2md.mineru_server.subprocess.Popen")
    ...
    @patch("document2md.mineru_server._wait_until_healthy", side_effect=RuntimeError("never healthy"))
    @patch("document2md.mineru_server.subprocess.Popen")
